...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error(err);
      showToast(err instanceof Error ? err.message : '解析 ZIP 备份文件失败，请确认文件格式', 'error');
    } finally {
  ...
  return (
    ...
    >
      ...
      <div
        className="relative w-full sm:max-w-md bg-[#FAF7F2] rounded-t-3xl sm:rounded-2xl border border-[#E6E0D6] shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-in slide-in-from-bottom sm:zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header with warm aesthetic stamp */}
        <div className="px-4 py-2 border-b border-[#EAE3D6] flex items-center justify-between bg-[#F4EFEA]/80">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg bg-[#E8DFD3] border border-[#D9CFC1] flex items-center justify-center text-[#524436]">
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-1 rounded-lg text-[#8C7E70] hover:text-[#382F26] hover:bg-black/5 transition-colors"
            title="关闭"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        {/* Tab Navigation */}
        <div className="flex items-center px-4 pt-1 border-b border-[#EAE3D6] gap-3 bg-[#FAF7F2] overflow-x-auto scrollbar-none">
          <button
            type="button"
          ...
          {activeTab === 'storage' && (
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="font-serif font-bold text-sm text-[#382F26]">手账全量导出与数据导入</h4>
                  <span className="text-[11px] text-[#4A5D4E] font-medium bg-[#E8F0E8] px-2 py-0.5 rounded-md border border-[#D0E0D0]">全量内容模式</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  >
                    ...
                    <span>{isParsingFile ? '解析文件中...' : '导入手账备份文件 (全量恢复)'}</span>
                  </button>
                </div>
                <p className="text-[11px] text-[#9E9082] leading-relaxed">
                  支持按需选择单本或全部手账导出。导出将打包全部文字便签、排版布局、高清原图与视频；导入时将完整复原并写入本地存储，确保手账跨设备无损迁移。
                </p>
              </div>
