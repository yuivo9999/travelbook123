...
export const TextItem: React.FC<TextItemProps> = ({
  ...
  const handleSave = () => {
    if (draftText.trim() !== (item.text || '').trim()) onUpdateText(item.id, draftText.trim());
    setIsEditing(false);
  };
  ...
  const handleResizePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    let newW = Math.max(80, Math.min(Math.max(80, canvasWidth - 16), state.initialWidth + localDw));
    let newH = Math.max(80, Math.min(1200, state.initialHeight + localDh));
    if (newW !== state.initialWidth || newH !== state.initialHeight) state.moved = true;
    if (typeof window !== 'undefined') {
      // Keep the persisted ContentItem object in sync so NotebookView's drag bounds use the real width.
  ...
  const handleResizePointerUp = async (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (state.moved) {
      await updateItemTransform(item.id, { width: item.width, height: item.height });
    }
  };
  ...
  return (
    <div
      id={`item-${item.id}`}
      style={{ transform: `translate3d(${item.x}px, ${item.y}px, 0px) rotate(${item.rotation || 0}deg)`, width: `${effectiveWidth}px`, zIndex: item.zIndex }}
      onPointerDownCapture={() => onBringToFront?.(item.id)}
    ...
    >
      <div className={`relative rounded-xl border p-3 pt-2 shadow-[var(--scrap-shadow)] hover:shadow-[var(--scrap-hover)] transition-all flex flex-col ${colorStyles}`} style={{ minHeight: `${effectiveHeight}px` }}>
        <div className="flex flex-wrap items-center justify-between gap-1 pb-1 mb-1 border-b border-black/5">
          <div onPointerDown={(e) => onDragStart(e, item)} className="flex-1 min-w-[32px] flex items-center justify-center py-1 cursor-grab active:cursor-grabbing touch-none select-none text-[#94887C] hover:text-[#4A3F35]" title="按住拖拽移动便签位置">
          ...
          <div className="flex flex-wrap items-center justify-end gap-0.5 opacity-80 group-hover:opacity-100 transition-opacity">
            {!isEditing ? <>
              <button type="button" onClick={() => setIsEditing(true)} className="p-1 rounded-md text-[#7D7062] hover:text-[#2D2721] hover:bg-black/5 transition-colors" title="编辑文字"><Edit3 className="w-3.5 h-3.5" /></button>
              <button type="button" onClick={() => onDelete(item.id)} className="p-1 rounded-md text-[#A85B5B] hover:text-[#C5221F] hover:bg-black/5 transition-colors" title="删除此便签"><Trash2 className="w-3.5 h-3.5" /></button>
            </> : <>
              <button type="button" onClick={handleSave} className="p-1 rounded-md text-[#198754] hover:bg-black/5 transition-colors" title="完成编辑"><Check className="w-3.5 h-3.5" /></button>
              <button type="button" onClick={handleCancel} className="p-1 rounded-md text-[#6C757D] hover:bg-black/5 transition-colors" title="取消"><X className="w-3.5 h-3.5" /></button>
            </>}
          </div>
        </div>

        {isEditing ? (
          <div className="pt-1 flex-1 flex flex-col min-h-[80px]">
            <textarea ref={textareaRef} value={draftText} onChange={(e) => setDraftText(e.target.value)} onBlur={handleSave} className="w-full flex-1 min-h-[70px] bg-transparent border-0 focus:outline-hidden text-sm leading-relaxed resize-none select-text font-normal" placeholder="输入文字..." />
            <div className="flex justify-end pt-1"><button type="button" onMouseDown={(e) => { e.preventDefault(); handleSave(); }} className="px-2.5 py-1 text-xs font-medium text-white bg-[#4A4036] rounded-md active:scale-95 transition-transform">保存</button></div>
          </div>
        ) : (
          <div onDoubleClick={() => setIsEditing(true)} className="text-sm leading-relaxed whitespace-pre-wrap break-words py-1 select-text font-normal cursor-text flex-1 flex flex-col justify-start">{item.text || '（空白便签，双击输入文字）'}</div>
        )}
