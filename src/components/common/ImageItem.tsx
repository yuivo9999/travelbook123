...
export const ImageItem: React.FC<ImageItemProps> = ({ item, canvasWidth, onDragStart, onRotateStart, onResizeStart, onBringToFront, onViewImage, onDelete }) => {
  ...
  // The stored width is the source of truth. Do not mutate it when the drag handle is pressed:
  // doing so previously caused an ordinary click on the handle to shrink legacy images.
    ...
    <div id={`item-${item.id}`} style={{ transform: `translate3d(${item.x}px, ${item.y}px, 0px) rotate(${item.rotation || 0}deg)`, width: `${effectiveWidth}px`, zIndex: item.zIndex }} onPointerDownCapture={() => onBringToFront?.(item.id)} className="absolute top-0 left-0 transition-shadow duration-150 group touch-auto select-none">
      <div className="relative bg-[#FFFFFF] rounded-xl border border-[#E6E0D6] p-2.5 shadow-[var(--scrap-shadow)] hover:shadow-[var(--scrap-hover)] transition-all flex flex-col" style={{ minHeight: `${effectiveHeight}px` }}>
        ...
        <div className="relative w-full flex-1 min-h-[70px] bg-[#F5F2ED] rounded-lg overflow-hidden flex items-center justify-center group/img" onClick={(e) => e.stopPropagation()}>
          {loading ? <div className="flex flex-col items-center gap-1 text-[#A09386]"><ImageIcon className="w-6 h-6 animate-pulse" /><span className="text-[10px]">读取图片...</span></div> : thumbUrl && !hasError ? <img src={thumbUrl} alt="手账照片" loading="lazy" onError={handleImageError} className="w-full h-full object-cover select-none transition-transform duration-300 pointer-events-none" draggable={false} /> : <div className="flex flex-col items-center gap-1 text-[#9E9082] p-4 text-center"><ImageIcon className="w-6 h-6 opacity-60" /><span className="text-xs">暂无图片数据</span></div>}
        </div>
