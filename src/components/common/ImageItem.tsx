...
import { GripHorizontal, Trash2, Maximize2, Image as ImageIcon, Link2, RotateCw } from 'lucide-react';
...
export const ImageItem: React.FC<ImageItemProps> = ({ item, canvasWidth, onDragStart, onRotateStart, onBringToFront, onViewImage, onDelete }) => {
  ...
  const effectiveWidth = Math.min(Math.max(item.width || 240, 80), Math.max(80, canvasWidth - 16));
  const effectiveHeight = Math.max(item.height || 200, 80);

  // Keep image resizing local to the item, just like the text/video items.
  // This deliberately does not touch NotebookView's legacy minimum-size logic.
  const handleResizePointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    resizeRef.current = {
      ...
      rotationRad: ((item.rotation || 0) * Math.PI) / 180,
      moved: false,
    };
  };
  ...
  const handleResizePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const maxWidth = Math.max(80, canvasWidth - 16);
    const newW = Math.max(80, Math.min(maxWidth, state.initialWidth + localDw));
  ...
  return (
    <div id={`item-${item.id}`} style={{ transform: `translate3d(${item.x}px, ${item.y}px, 0px) rotate(${item.rotation || 0}deg)`, width: `${effectiveWidth}px`, zIndex: item.zIndex }} onPointerDownCapture={() => onBringToFront?.(item.id)} className="absolute top-0 left-0 transition-shadow duration-150 group touch-auto select-none">
      <div className="relative bg-[#FFFFFF] rounded-xl border border-[#E6E0D6] p-2.5 shadow-[var(--scrap-shadow)] hover:shadow-[var(--scrap-hover)] transition-all flex flex-col overflow-visible" style={{ height: `${effectiveHeight}px`, minHeight: '80px' }}>
        <div className="flex flex-wrap items-center justify-between gap-1 pb-1.5 border-b border-black/5 mb-1.5 shrink-0">
        ...
        <div className="relative w-full flex-1 min-h-0 bg-[#F5F2ED] rounded-lg overflow-hidden flex items-center justify-center group/img">
          {loading ? <div className="flex flex-col items-center gap-1 text-[#A09386]"><ImageIcon className="w-6 h-6 animate-pulse" /><span className="text-[10px]">读取图片...</span></div> : thumbUrl && !hasError ? <img src={thumbUrl} alt="手账照片" loading="lazy" onError={handleImageError} className="w-full h-full object-cover select-none transition-transform duration-300 pointer-events-none" draggable={false} /> : <div className="flex flex-col items-center gap-1 text-[#9E9082] p-2 text-center"><ImageIcon className="w-6 h-6 opacity-60" /><span className="text-xs">暂无图片数据</span></div>}
        </div>
        {(fileMeta.fileName || fileMeta.sourceUrl) && <div className="mt-1.5 pt-1.5 border-t border-black/5 flex items-center justify-between text-[10px] text-[#827466] shrink-0"><a href={fileMeta.sourceUrl?.startsWith('http') ? fileMeta.sourceUrl : undefined} target={fileMeta.sourceUrl?.startsWith('http') ? '_blank' : undefined} rel="noreferrer" onClick={(e) => { if (!fileMeta.sourceUrl?.startsWith('http')) { e.preventDefault(); navigator.clipboard?.writeText(fileMeta.sourceUrl || fileMeta.fileName || ''); } }} className="flex items-center gap-1 max-w-full truncate hover:text-[#4A3F35] transition-colors" title={`原文件地址: ${fileMeta.sourceUrl || fileMeta.fileName || ''} (点击复制或打开)`}><Link2 className="w-3 h-3 shrink-0 opacity-70" /><span className="truncate">{fileMeta.fileName || fileMeta.sourceUrl}</span></a></div>}
