...
export const NotebookView: React.FC<NotebookViewProps> = ({
  ...
  const [addModalInitialTab, setAddModalInitialTab] = useState<'menu' | 'text' | 'url'>('menu');
  ...
  const getNextSpawnCoordinates = (itemWidth = 260, itemHeight = 180) => {
    ...
    // Always straight (0 degrees) when newly added
    const rotation = 0;
  ...
  const handleAddImage = async (file: File) => {
    try {
      ...
      // Compute display dimensions for scrapbook card (max 50% of notebook page width)
      const maxHalfWidth = Math.max(160, Math.round(canvasWidth * 0.5));
      const cardWidth = Math.min(260, maxHalfWidth, canvasWidth - 24);
  ...
  const handleAddVideo = async (file: File) => {
    try {
      ...
      const maxHalfWidth = Math.max(160, Math.round(canvasWidth * 0.5));
      const cardWidth = Math.min(280, maxHalfWidth, canvasWidth - 24);
  ...
  const handleDelete = useCallback((id: string, mediaId?: string) => {
    setItemToDelete({ id, mediaId });
  }, []);

  const handleViewImage = useCallback((mediaId?: string) => {
    setPreviewImageId(mediaId || null);
  }, []);

  const handlePlayVideo = useCallback((mediaId?: string) => {
    setPreviewVideoId(mediaId || null);
  }, []);

  const handleOpenWebpage = useCallback((url: string, title?: string) => {
    setPreviewWebUrl({ url, title });
  }, []);
  ...
  const handleDragStart = useCallback((e: React.PointerEvent, item: ContentItem) => {
    e.stopPropagation();
    e.preventDefault();

    if (item.isPinned) {
      showToast('此窗口已被钉住，无法移动。请点击右上角钉子按钮解开。', 'info');
      return;
    }
  ...
  const handleRotateStart = useCallback((e: React.PointerEvent, item: ContentItem) => {
    e.stopPropagation();
  ...
  }, [settings.rotationSoundVolume, settings.enableRotationSound]);

  // Resize interaction start (for customizable notes)
  const handleResizeStart = useCallback((e: React.PointerEvent, item: ContentItem) => {
    e.stopPropagation();
  ...
  const handleResetTransform = useCallback(async (item: ContentItem) => {
  ...
  }, [settings.rotationSoundVolume, settings.enableRotationSound, showToast]);

  const handleTogglePin = useCallback(async (item: ContentItem) => {
    ...
    try {
      ...
      showToast(newPinnedState ? '窗口已钉住（位置已锁定）' : '窗口已解开钉子', 'success');
    } catch (err) {
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    if (rotatingRef.current || resizingRef.current || draggingRef.current) {
      e.preventDefault();
    }
    ...
    if (draggingRef.current) {
      ...
      // Edge auto-scrolling when card is dragged to top or bottom viewport edges
      const edgeThreshold = 80;
      if (e.clientY < edgeThreshold) {
        window.scrollBy({ top: -10, behavior: 'auto' });
        draggingRef.current.startY -= 10;
      ...
      draggingRef.current.targetX = newX;
      draggingRef.current.targetY = newY;

      // Direct DOM transform update for ultra-smooth 60/120fps zero-lag dragging
      const el = document.getElementById(`item-${itemId}`);
      if (el) {
        const rotation = item?.rotation || 0;
        el.style.transform = `translate3d(${newX}px, ${newY}px, 0px) rotate(${rotation}deg)`;
      }
    }
  };
  ...
  const handlePointerUp = async (e: React.PointerEvent) => {
    ...
    // 3. Dragging finish
    if (draggingRef.current) {
      const { itemId, hasMoved, pointerId, targetX, targetY } = draggingRef.current;
      ...
      if (hasMoved && targetX !== undefined && targetY !== undefined) {
        setItems((prev) =>
          prev.map((it) => (it.id === itemId ? { ...it, x: targetX, y: targetY } : it))
        );
        const current = items.find((i) => i.id === itemId);
        if (current) {
          // Auto-save position to IndexedDB
          await updateItemPosition(current.id, targetX, targetY, current.zIndex);
        }
      }
    }
  };
  ...
  return (
    ...
    >
      ...
      <header className="sticky top-0 z-40 bg-[#FAF7F2]/95 backdrop-blur-md border-b border-[#E5DFD5] px-3 sm:px-6 py-2.5 shadow-xs safe-area-top">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            <button
              ...
              title="导出这本手账 (全量内容 JSON 格式，含完整文字、原图与原视频)"
            >
      ...
      <AddContentModal
        ...
        onPickImages={handleOpenImagePicker}
        onPickVideos={handleOpenVideoPicker}
      />
      ...
      <EditCoverModal
        ...
        onSave={async (coverData) => {
          const updatedNotebook = { ...notebook, ...coverData, updatedAt: Date.now() };
