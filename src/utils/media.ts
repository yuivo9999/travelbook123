...
import { readOriginalMedia, writeOriginalMedia } from './mediaStorage';


const MAX_THUMBNAIL_BYTES = 1024 * 1024;

function createImageFallbackThumbnail(): Blob {
  try {
    ...
    if (ctx) {
      ...
      ctx.fillText('图片预览', 240, 155);
      ctx.font = '12px sans-serif';
      ctx.fillText('原图已安全保存', 240, 180);
...
function canvasToBlob(canvas: HTMLCanvasElement): Blob | null {
  // Synchronous helper is not available for canvas.toBlob; use data URL for fallback only.
  try {
    return ensureBlob(canvas.toDataURL('image/jpeg', 0.72), 'image/jpeg');
  } catch {
    return null;
  }
}

function safeThumbnailFallback(raw: Blob): Blob {
  return raw.size <= MAX_THUMBNAIL_BYTES ? raw : createImageFallbackThumbnail();
}
...
export async function processImageFile(file: File | Blob, maxWidth = 640): Promise<ImageProcessResult> {
  return new Promise((resolve) => {
    ...
    try {
      ...
      resolve({ thumbnailBlob: safeThumbnailFallback(rawBlob as Blob), width: 400, height: 300 });
      return;
    }
    ...
    img.onload = () => {
      ...
      const ctx = canvas.getContext('2d');
      if (!ctx) {
        resolve({ thumbnailBlob: safeThumbnailFallback(rawBlob as Blob), width: originalWidth, height: originalHeight });
        return;
      }
...
// ----------------- Persistent Original Media Resolution -----------------

// In-session registry is only a short-lived performance cache, never the source of truth.
...
export async function getOriginalMediaBlob(media: any): Promise<Blob | null> {
  if (!media) return null;

  // 1. OPFS is the primary persistent source for original media.
  const opfsBlob = await readOriginalMedia(media.id, media.mimeType || 'application/octet-stream');
  if (opfsBlob) return opfsBlob;

  // 2. Legacy IndexedDB blob: migrate it to OPFS when possible.
  if (media.blob instanceof Blob && media.blob.size > 0) {
    try { await writeOriginalMedia(media.id, media.blob); } catch { /* compatibility fallback */ }
    return media.blob;
  }

  // 3. Current-session file cache. This is not persistent backup storage.
  const sessionFile = sessionFileRegistry.get(media.id);
  if (sessionFile && sessionFile.size > 0) return sessionFile;

  // 4. Legacy FileHandle recovery only. New records never depend on it.
  if (media.fileHandle) {
    try {
      const permission = typeof media.fileHandle.queryPermission === 'function'
        ? await media.fileHandle.queryPermission({ mode: 'read' })
        : 'granted';
      if (permission === 'granted' && typeof media.fileHandle.getFile === 'function') {
        const file = await media.fileHandle.getFile();
        if (file?.size > 0) return file;
      }
    } catch { /* legacy handle is unavailable */ }
  }
...
export async function resolveOriginalMedia(media: any): Promise<ResolvedMediaResult> {
  if (!media) {
    return { success: false, isMissing: true, fileName: '未知文件', error: 'missing 文件', message: '未找到媒体记录' };
  }
  ...
  if (media.sourceUrl && typeof media.sourceUrl === 'string') {
    ...
    if (trimmed.startsWith('http://') || trimmed.startsWith('https://') || trimmed.startsWith('data:') || trimmed.startsWith('blob:')) {
      return { success: true, url: trimmed, isObjectUrl: false, fileName, sourceUrl: trimmed };
    }
  }
