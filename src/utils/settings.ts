...
// Export all notebook and item metadata to a downloadable JSON file
export async function exportAllDataToJSON(): Promise<void> {
  ...
  downloadAnchor.setAttribute('download', `travelbook-backup-${dateStr}.json`);
  document.body.appendChild(downloadAnchor);
  downloadAnchor.click();
  downloadAnchor.remove();
}

// Import notebook and item metadata from JSON file
export async function importDataFromJSON(
  file: File
): Promise<{ notebookCount: number; itemCount: number }> {
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (!data || !Array.isArray(data.notebooks) || !Array.isArray(data.items)) {
          throw new Error('无效的备份文件格式，缺少手账或内容列表');
        }
        ...
        reject(err instanceof Error ? err : new Error('解析导入文件失败'));
      }
    };
